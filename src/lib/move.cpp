...
void move(int day)
{
    ...
    {
        return virtualServerList[A].core + virtualServerList[A].ram >
               virtualServerList[B].core + virtualServerList[B].ram;
    };
    ...
    for (auto &i : canMoveList)
    {
        for (auto j = rate.begin(); j != rate.end(); j++)
        {
            ...
            {
                if (Ser.lCore > virSer.core / 2 && Ser.lRam > virSer.ram / 2 && Ser.rCore > virSer.core / 2 &&
                    Ser.rRam > virSer.ram / 2 && virSer.serverId != Ser.id)
                {
