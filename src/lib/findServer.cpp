...
int findServer(request &req, virtualServer &virSerType, bool isDouble)
{
        ...
        { // 查找可以塞的下的
            if (i.second.lCore >= virSerType.core && i.second.lRam >= virSerType.ram)
            {
                ...
                } else
                {
                    ...
                    if (i.second.lCore + i.second.rCore + i.second.lRam + i.second.rRam >
                        serverList[retServerId].lCore + serverList[retServerId].rCore + serverList[retServerId].lRam +
                        serverList[retServerId].rRam)
                        continue;
                    if (lorR == 0)
                    {
                        if (i.second.lCore + i.second.lRam <
                            serverList[retServerId].lCore + serverList[retServerId].lRam)
                        {
                            retServerId = i.first;
                            lorR = 0;
                        }
                    ...
                    {
                        if (i.second.lCore + i.second.lRam <
                            serverList[retServerId].rCore + serverList[retServerId].rRam)
                        {
                            retServerId = i.first;
                            lorR = 0;
                        }
            ...
            if (i.second.rCore >= virSerType.core && i.second.rRam >= virSerType.ram)
            {
                ...
                } else
                {
                    ...
                    if (lorR == 0)
                    {
                        if (i.second.rCore + i.second.rRam <
                            serverList[retServerId].lCore + serverList[retServerId].lRam)
                        {
                            retServerId = i.first;
                            lorR = 1;
    ...
    } else
    {
        for (auto &i : serverList)
        {
            if (i.second.lCore >= virSerType.core / 2 && i.second.lRam >= virSerType.ram / 2 &&
                i.second.rCore >= virSerType.core / 2 && i.second.rRam >= virSerType.ram / 2)
            {
                // 策略零
